...
#c.JupyterHub.data_files_path = '/opt/anaconda3/envs/mibaenv/share/jupyterhub'
...
#c.LocalGitHubOAuthenticator.oauth_callback_url = 'https://jhas00.gsom.spbu.ru/hub/oauth_callback'
#c.LocalGitHubOAuthenticator.client_id = '49aa02657abecfa1e29c'
#c.LocalGitHubOAuthenticator.client_secret = '815cf110e14d525c91d93c896c86ddafa50d78c8'
...
class MibaLocalAzureAdOAuthenticator(LocalAzureAdOAuthenticator):
    ...
    def add_system_user(self, user):
        ...
        if name in self.admin_users:
            cmd = ['usermod', '-a', '-G', 'mibaadmins', name]
            self.log.info("Adding user to admin group: %s", ' '.join(map(pipes.quote, cmd)))
            p = Popen(cmd, stdout=PIPE, stderr=STDOUT)
            p.wait()
            if p.returncode:
                err = p.stdout.read().decode('utf8', 'replace')
                raise RuntimeError("Failed to add user to admin group %s: %s" % (name, err))
        cmd = ['usermod', '-a', '-G', 'mibastudents', name]
        self.log.info("Adding user to a group: %s", ' '.join(map(pipes.quote, cmd)))
        p = Popen(cmd, stdout=PIPE, stderr=STDOUT)
        p.wait()
        if p.returncode:
            err = p.stdout.read().decode('utf8', 'replace')
            raise RuntimeError("Failed to add user to a group %s: %s" % (name, err))
        cmd = ['ln', '-s', '/home/shared', f'/home/{name}']
        self.log.info("Adding link to shared folder for user: %s", ' '.join(map(pipes.quote, cmd)))
        p = Popen(cmd, stdout=PIPE, stderr=STDOUT)
        p.wait()
        if p.returncode:
            err = p.stdout.read().decode('utf8', 'replace')
            raise RuntimeError("Failed to add link to shared folder for user %s: %s" % (name, err))
        cmd = ['ln', '-s', '/home/data', f'/home/{name}']
        self.log.info("Adding link to data folder for user: %s", ' '.join(map(pipes.quote, cmd)))
        p = Popen(cmd, stdout=PIPE, stderr=STDOUT)
        p.wait()
        if p.returncode:
            err = p.stdout.read().decode('utf8', 'replace')
            raise RuntimeError("Failed to add link to data folder for user %s: %s" % (name, err))

with open('/etc/jupyterhub/params.json') as file:
    PARAMS = json.load(file)

c.JupyterHub.authenticator_class = MibaLocalAzureAdOAuthenticator

c.MibaLocalAzureAdOAuthenticator.tenant_id = PARAMS['tenant_id']
c.MibaLocalAzureAdOAuthenticator.oauth_callback_url = 'https://jhas00.gsom.spbu.ru/hub/oauth_callback'
c.MibaLocalAzureAdOAuthenticator.client_id = PARAMS['client_id']
c.MibaLocalAzureAdOAuthenticator.client_secret = PARAMS['client_secret']
c.MibaLocalAzureAdOAuthenticator.username_claim = 'unique_name'
...
c.LocalAuthenticator.admin_users = set(PARAMS['admin_users'])
...
#c.Authenticator.admin_users = set({'jhubadmin'})
